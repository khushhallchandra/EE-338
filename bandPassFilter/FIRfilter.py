...
f_digital = np.array([omega_s1,omega_p1,omega_p2,omega_s2],dtype='f')
...
iterable = ((np.sin(w_cut_2*k)-np.sin(w_cut_1*k))/(np.pi*k) for k in range(int(-N),int(N+1)))
h_ideal=np.fromiter(iterable,float)
h_ideal[N]=((w_cut_2-w_cut_1)/np.pi)
